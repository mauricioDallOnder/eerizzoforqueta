...
export default function ControleFrequenciaTable({
  ...
  const countMonthlyAbsence = (presencas: Record<string, Record<string, boolean>>, month: string): number => {
    const monthPresences = presencas[month];
    return monthPresences ? Object.values(monthPresences).filter(presence => !presence).length : 0;
  };
            ...
            <TableRow>
              ...
              {months.map((month) => (
                <TableCell key={month} sx={{ border: "1px solid black" }} align="right">
                  {month.charAt(0).toUpperCase() + month.slice(1)} {/* Capitalize month names */}
                </TableCell>
              ))}
            </TableRow>
          </TableHead>
              ...
              alunosDaTurma.map((aluno: Aluno) => (
                <TableRow key={aluno.id}>
                  ...
                  {months.map((month, monthIndex) => (
                    <TableCell key={monthIndex} sx={{ border: "1px solid black" }} align="center">
